...
FROM --platform=$BUILDPLATFORM docker.io/library/golang:1.26.2-bookworm AS server-builder
...
FROM --platform=$BUILDPLATFORM docker.io/library/golang:1.26.2-bookworm AS healthcheck-builder
COPY --from=xx / /
ARG TARGETPLATFORM
RUN xx-apt install -y libc6-dev binutils gcc libc6-dev
